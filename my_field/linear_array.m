...
% [phantom_positions, phantom_amplitudes] = cyst_phantom(1000);  %计算散斑
...
[v, t1]=calc_scat(Th, Th2,positions,amp);
%  Store the result
...
ni=1:0.2:length(x);
z=1:length(depth);
zi=1:0.2:length(depth);
xi=interp1(n,x,ni,'linear');
depthi=interp1(z,depth,zi,'linear');    %拓展成像数据点数，四倍插值


env_gray_interpt=interp2(env_gray,5);  %zi=interp2(Z,n);做n次递归计算，在两元素间插入二维插值

%ZI = interp2(X,Y,Z,XI,YI,method)   用指定的算法method 计算二维插值：
%[X, Y] = meshgrid(x,y);
%% %%%%%%%%%%%%%%%%%%%%%%%
image(xi*1000, depthi*1000, env_gray_interpt)
